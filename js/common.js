...
let addheader = function (headers) {
    ...
    for (var i = 0; i < headers.length; i++) {
        ...
        let source = 'https://sdat-dev.github.io/healthequity/assets/images/' + (typeof headers[i].source != 'undefined' && headers[i].source != '' ? headers[i].source + '/' : '');
        if (i == 0) {
        ...
        if (header1 == '') {
            ...
            content += '<img src="' + source + image + '" class="d-block w-100" alt="...">' +
                ...
                '<h1>' + header1 + '</h1>' +
                '<p>' + header2 + '</p>' +
                // '<button class="button-properties"><a href="/healthequity/jobopportunity.html">Job Opening</a></button>' +
                '</div>' +
                '</div>';
        }
    }
